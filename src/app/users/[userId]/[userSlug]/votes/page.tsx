    ...
    votes.documents = await Promise.all(
        votes.documents.map(async vote => {
            try {
                ...
                console.log(`Error fetching document for vote ${vote.$id}:`, error);
                // Return vote with placeholder question if documents are missing
                return {
                    ...vote,
                    question: {
                        $id: "unknown",
                            ...
                            <div className="flex">
                                ...
                                <p>
                                    {vote.question.$id !== "unknown" ? (
                                        ...
                                        <span className="text-gray-500">{vote.question.title}</span>
                                    )}
                                </p>
                            </div>
                    ...
                    <div className="rounded-lg border border-gray-200 bg-gray-50 p-8 text-center dark:border-gray-700 dark:bg-gray-800">
                        ...
                        <p className="text-gray-600 dark:text-gray-300">
                            {searchParams.voteStatus 
                                ? `No ${searchParams.voteStatus.replace('d', '')}s yet.`
                                : "This user hasn't voted on any questions or answers yet."}
                        </p>
                    </div>
                )}
            </div>
